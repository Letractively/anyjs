...
/* 			if(!anyjs.temp){anyjs.temp=true;
	...
	get: function(str){//已测试通过Trident,Webkit,Geckos //此时参数只能是ID值，只可一个唯一
	...
	sout: function(speed,direct){ 
	 ...
	 if(anyjs.temp!=null) clearInterval(anyjs.temp);
	 anyjs.temp=setInterval(function(){val-=speed;that.cs(direct,val+"px");if(val<=0) {clearInterval(anyjs.temp); anyjs.temp=null; that.hide();}},1);
	 },
	 ...
	 that.show();
	 anyjs.temp=setInterval(function(){val+=speed;that.cs(direct,val+"px");if(val>=anyjs.globalval[id+'slide']) {clearInterval(anyjs.temp); anyjs.temp=null; }},1);
	 }, 
	/****内部使用方法****/
	...
	addEvent: function(elm,evType,fn){ //已测试通过Trident,Webkit,Geckos 内部使用
	...
	rmvEvent: function(ele, evtype, handler) { //已测试通过Trident,Webkit,Geckos 内部使用
		...
		if(xhr.readyState == 1) {	//alert("1");
			...
			if(xhr.status == 200) {
				if(datatype=="XML"||datatype=="xml") {
					...
					callbackfn(xhr.responseXML);}
				else {//alert(xhr.responseHTML);
