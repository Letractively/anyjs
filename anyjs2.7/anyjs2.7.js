...
var _ANY=function(ele){
   ...
   //return this.elements[0];  //1 $("id").innerHTML 失效 2 //无返回值 3 实例化时执行构造
};//end function _ANY()
...
/* 	init: function(){
	...
	msmv: function(fn){ //已测试通过T,W,G
 	...
 	toggle: function(fn1,fn2){ //目前不可用
			...
			if(anyjs.globalvar[id+'togglefunc']==undefined||anyjs.globalvar[id+'togglefunc']==fn2) 
	...
	get: function(str){//通过T,W,G //此时参数唯一//在IE下可以获取动态value值FF,chrome下失效，需用$$().value代替
		var attr=str==undefined?"value": str;
		 ...
		 //if(str=="value") return this.elements[0].value;//?失效
		return this.elements[0].getAttribute(attr);
	},
	set:function(attr,val){ //setattribute 通过T,W,G
        this.each(function(el){
			if(attr== 'html'||attr=="h"){
				el.innerHTML = val;
	...
	rmvcls:function(clsname){ //通过T,W,G
        this.each(function(el){
			el.className=el.className.replace(clsname,"");
        });
    return this;
    },
	h: function(data){ //通过T,W,G
		var that=this;
        this.each(function(){
        that.set("h",data);
        })
	...
	fin: function(){ //通过T,W,G
	 var opval=1,that=this; //不同于sin(),勿需函数保护,闪屏可用作提示
	 if(anyjs.temp!=null) clearInterval(anyjs.temp);
	...
	sout: function(speed,direct){ 
	 ...
	 if(anyjs.globalvar[id+'slide']==undefined) anyjs.globalvar[id+'slide']=val;//防止反复点击时，val的原始值被篡改//alert(id+val+anyjs.globalvar[id+'slide']);
	 if(anyjs.temp!=null) clearInterval(anyjs.temp);
	 ...
	 if(anyjs.globalvar[id+'slide']==undefined) return false;//当未执行sout()时，此操作将无效，需注意不是同一ID的情况
	 speed=(speed==undefined?5: speed);
	 direct=(direct==undefined?"height": direct);
	 if(anyjs.temp!=null) clearInterval(anyjs.temp);
	...
	addEvent: function(el,evType,fn){ //通过T,W,G 内部
		   anyjs.globalvar["e"+evType+fn] = fn;  
		   if (el.addEventListener) { 
				el.addEventListener(evType, anyjs.globalvar["e"+evType+fn], false); //DOM2.0 useCapture默认false,捕获+冒泡
				return true; 
		   } 
			else if (el.attachEvent) {
				el.attachEvent( "on"+evType, function() {anyjs.globalvar["e"+evType+fn](); });  
			} 
	},
	rmvEvent: function(evType, fn) { //globalvar ok 
		this.each(function(el){
			if (el.removeEventListener) {
				el.removeEventListener(evType, anyjs.globalvar["e"+evType+fn], false);
			} else {
				el.detachEvent( "on"+evType, anyjs.globalvar["e"+evType+fn] ); 
				anyjs.globalvar["e"+evType+fn] = null;  //alert("running");
			}
		});
	return this;
  }//end rmvEvent
